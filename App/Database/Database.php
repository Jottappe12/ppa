...
class Database
{
  ...
  {
    try {
      $this->connection = new PDO('firebird:dbname=' . self::$host . self::$name . ';port=' . self::$port, self::$user, self::$pass);
  ...
  public function select($where = null, $order = null, $limit = null, $fields = '*')
  {
    ...
    $limit = $limit ? 'FIRST ' . $limit[1] . ' SKIP ' . $limit[0] . ' ' : '';
    //MONTA A QUERY
    $query = 'SELECT ' . ' ' . $limit . ' ' . $fields . ' FROM ' . $this->table . ' ' . $where . ' ' . $order;
  ...
  public function update($where, $values)
  {
    ...
    //EXECUTAR A QUERY
    if ($this->execute($query, array_values($values))) return true;
  }
